...
@RestController({
  secure: true,
  resource: 'ownpermission',
})
export class PermissionOwnController {
  constructor(private readonly dataService: PermissionService) {}

  @PostOwnReq()
  createOwn(
    @ReqBody() createDTO: CreatePermissionDto,
  ...
  @GetOwnReq()
  findAllOwn(
    @ReqQuery() query: QueryDTO<Permission>,
    @ReqUser() user: IUserEntity
  ) {
  ...
  updateOwn(
    @IDParam() id: number,
    @ReqBody() updateDTO: UpdatePermissionDto,
    @ReqUser() user: IUserEntity
  ) {
  ...
  async softDeleteOwn(
    @IDParam() id: number,
    @Query() query: QueryDTO<Permission>,
    @ReqUser() user: IUserEntity
  ) {

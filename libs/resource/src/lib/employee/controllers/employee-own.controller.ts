...
@RestController({
  secure: true,
  resource: 'ownemployee',
})
export class EmployeeOwnController {
  constructor(private readonly dataService: EmployeeService) {}

  @PostOwnReq()
  createOwn(
    @ReqBody() createDTO: CreateEmployeeDto,
  ...
  @GetOwnReq()
  findAllOwn(
    @ReqQuery() query: QueryDTO<Employee>,
    @ReqUser() user: IUserEntity
  ) {
  ...
  updateOwn(
    @IDParam() id: number,
    @ReqBody() updateDTO: UpdateEmployeeDto,
    @ReqUser() user: IUserEntity
  ) {

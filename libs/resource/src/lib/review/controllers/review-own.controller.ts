...
export class ReviewOwnController {
  constructor(private readonly dataService: ReviewService) {}

  @PostOwnReq()
  createOwn(
    @ReqBody() createDTO: CreateReviewDto,
  ...
  @GetOwnReq()
  findAllOwn(
    @ReqQuery() query: QueryDTO<Review>,
    @ReqUser() user: IUserEntity
  ) {
  ...
  updateOwn(
    @IDParam() id: number,
    @ReqBody() updateDTO: UpdateReviewDto,
    @ReqUser() user: IUserEntity
  ) {

...
export class CommentOwnController {
  constructor(private readonly dataService: CommentService) {}

  @PostOwnReq()
  createOwn(
    @ReqBody() createDTO: CreateCommentDto,
  ...
  @GetOwnReq()
  findAllOwn(
    @ReqQuery() query: QueryDTO<Comment>,
    @ReqUser() user: IUserEntity
  ) {
  ...
  updateOwn(
    @IDParam() id: number,
    @ReqBody() updateDTO: UpdateCommentDto,
    @ReqUser() user: IUserEntity
  ) {
